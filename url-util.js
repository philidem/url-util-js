/**
 * Module definition for amd-router that works in AMD, Node.js, and browser environment.
 */
(function() {

    function ModuleFactory(require) {
    ...
    // The following block of code normalizes module definition across Node.js, AMD, and browser
    var thisModule = undefined,
        moduleName = 'url-util';

    if (typeof module !== 'undefined') {
        // we're in Node environment so go ahead and build module and export it
    ...
    if (typeof define !== 'undefined') {
        ...
        if (thisModule) {
            // Use the module that we already instantiated in Node environment
            define(moduleName, function(require) {
                return thisModule;
            });
        } else {
            // Define module using ModuleFactory which will build module on demand
            define(moduleName, ModuleFactory);
        }
    } else if ((thisModule === undefined) && (typeof window !== 'undefined')) {
        // We're not in Node environment and AMD is not available but there is a window object.
        // Export via window object
        window.urlUtil = ModuleFactory(require);
    }
})();