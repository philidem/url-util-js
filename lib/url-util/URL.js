...
define('url-util/URL', function() {
    ...
    URL.prototype = {
        _parse : function(url) {
             ...
             * 
             * if (hashPos !== -1) { this.hash = url.substring(hashPos + 1); url =
             * url.substring(0, hashPos); }
             * 
             * var QueryString = require('url-util/QueryString');
             * 
             * var queryStringPos = url.indexOf('?'); if (queryStringPos == -1) { //
             * URL does not have a query string this.path = url;
             * 
             * if (queryString) { this.queryString =
             * QueryString.create(queryString); } } else { this.path =
             * url.substring(0, queryStringPos); this.queryString =
             * QueryString.parse(queryString);
             * this.queryString.parse(url.substring(queryStringPos + 1)); }
             */
